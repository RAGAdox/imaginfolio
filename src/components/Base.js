  ...
  return (
    <div>
      <div className={`w-full h-screen  ${loaded ? "" : "hidden"}`}>
        <img
          src={bgLeaf}
          className="w-full h-screen -z-10 fixed inset-0 object-cover object-center animate-bg-image-fade-in"
          onLoadCapture={(d) => setOnLoaded(!loaded)}
        />
        <div className="fixed top-0 left-0 z-50 w-full lg:w-auto">
          <Navbar />
        </div>
        <div className="w-full h-screen fixed backdrop-filter backdrop-blur-sm" />
        <div className="w-full h-screen overflow-hidden px-10 py-5 scroll-smooth	">
          {children}
        </div>
      </div>
      <div
